...
<style>
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .header { text-align: center; border-bottom: 3px double #7c3aed; padding-bottom: 16px; margin-bottom: 24px; }
    .org-name { font-size: 22px; font-weight: 700; color: #5b21b6; letter-spacing: 1px; }
    ...
    .watermark { position: fixed; bottom: 80px; right: 40px; font-size: 48px; color: rgba(124,58,237,.05); font-weight: 900; transform: rotate(-30deg); }
    .note { font-size: 11.5px; color: #64748b; border-top: 1px solid #e2e8f0; margin-top: 24px; padding-top: 10px; }
</style>
</head>
<body>
<div class="page">
    <div class="watermark">CHARACTER</div>
    ...
    <div class="cert-title"><h2>CHARACTER CERTIFICATE</h2></div>

    <div class="cert-no">
        Cert No: CC/{{ $student->admission_no }}/{{ now()->format('Y') }} &nbsp;&nbsp;|&nbsp;&nbsp; Date: {{ $generatedOn }}
    </div>
    ...
    <p class="body-text">
        ...
        Admission No. <span class="highlight">{{ $student->admission_no ?? '-' }}</span>,
        was a student of <span class="highlight">{{ $org?->name ?? config('app.name') }}</span>
        in <span class="highlight">Class {{ $student->academicClass?->name ?? '___' }}</span>
    ...
    <p class="body-text">
        During the period of study at this institution, the student's conduct and character were observed to be
        <span class="highlight">Good</span>. The student was disciplined, sincere, and respectful towards the staff,
        fellow students, and institution. No case of misconduct or indiscipline was ever reported against the student.
    </p>
    ...
    <p class="note">* This certificate is issued in good faith based on institutional records and observations.</p>
</div>
</body>
</html>
